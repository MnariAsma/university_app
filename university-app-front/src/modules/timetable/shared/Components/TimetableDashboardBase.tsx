...
} from "../../Teacher/Apis/TimetableApi";
import type { TimetableSession } from "../../Teacher/Apis/TimetableApi";
import {
  DAYS,
  TIME_SLOTS,
  HOUR_HEIGHT,
...
type TimetableView = "teacher" | "student";

interface TimetableDashboardBaseProps {
  view: TimetableView;
}
...
const SessionCard = ({
  ...
  return (
    <Box
      onClick={isTeacherView && onClick ? () => onClick(session) : undefined}
      sx={{
        ...sessionCard(top, height, isSelected, colors),
    ...
    >
      ...
      {height > 60 && session.teacher && (
        <Typography sx={sessionSubText(isSelected)}>
          {session.teacher.firstName} {session.teacher.lastName}
        </Typography>
      )}
      {isStudentView && height > 76 && session.room && (
        <Typography sx={sessionSubText(isSelected)}>
          {session.room.name}
        </Typography>
      )}
      {isStudentView && height > 92 && (
        <Typography sx={sessionSubText(isSelected)}>
          {session.type}
        </Typography>
      )}
...
const TimetableDashboardBase = ({ view }: TimetableDashboardBaseProps) => {
  ...
  const teacherQuery = useGetTeacherTimetableQuery(
    view === "teacher" ? weekStartStr : skipToken,
  );
  const studentQuery = useGetStudentTimetableQuery(
    view === "student" ? weekStartStr : skipToken,
  );
  const { data, isLoading, isError } = view === "teacher" ? teacherQuery : studentQuery;
  ...
  const dayColWidth = `calc((100% - ${TIME_COL_WIDTH}px) / ${DAYS.length})`;
          ...
          <Box sx={gridContainer}>
            <Box sx={dayHeadersRow}>
              ...
              {dayHeaders.map(({ day, label, isToday }) => (
                <Box key={day} sx={{ ...dayHeaderCell(isToday), width: dayColWidth, flexShrink: 0 }}>
                  <Typography
                    variant="caption"
            ...
            <Box sx={gridBody}>
              ...
              {dayHeaders.map(({ day, isToday }) => (
                <Box key={day} sx={{ ...dayColumn(isToday), width: dayColWidth }}>
                  {TIME_SLOTS.map((hour) => (
                  ...
                  {(sessionsByDay[day] ?? []).map((session) => (
                    <SessionCard
                      ...
                      session={session}
                      view={view}
                      onClick={view === "teacher" ? setSelectedSession : undefined}
                      isSelected={view === "teacher" && selectedSession?.id === session.id}
                    />
                  ))}
                </Box>
              ))}
...
export default TimetableDashboardBase;
